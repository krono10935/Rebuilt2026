...
public class ExponentialCommandXboxController extends CommandXboxController {
    private final ExponentialXboxController m_hid;

    private ControllerMultiplierType exponent;

    private final LoggedDashboardChooser<ControllerMultiplierType> exponentChooser;
    /**
     * Construct an instance of a controller.
     *
     * @param port The port index on the Driver Station that the controller is plugged into.
     */
    public ExponentialCommandXboxController(int port, ControllerMultiplierType exponent) {
        super(port);
        m_hid = new ExponentialXboxController(port,exponent);
        this.exponent = exponent;

        exponentChooser = new LoggedDashboardChooser<>("controller " + port + " exponent");
        configureChooser();
    }

    public void configureChooser(){
        exponentChooser.addDefaultOption(ControllerMultiplierType.LINEAR.name(), ControllerMultiplierType.LINEAR);
        Arrays.stream(ControllerMultiplierType.values()).forEach((exponent) -> {
            if(exponent != ControllerMultiplierType.LINEAR) exponentChooser.addOption(exponent.name(), exponent) ;
        });
        exponentChooser.onChange(m_hid::setExponent);
    }
     ...
     */
    @Override
    public ExponentialXboxController getHID() {
        return m_hid;
    }
